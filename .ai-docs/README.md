...
# ai-pipeline-core v0.13.0 — API Reference
...
class PipelineDeployment(Generic[TOptions, TResult]):
    ...
    def plan_next_flow(self, flow_class: type[PipelineFlow], plan: Sequence[PipelineFlow], output_documents: list[Document]) -> FlowDirective:
...
class Document(BaseModel, Generic[TContent]):
    ...
    def text(self) -> str:
        """Content decoded as UTF-8. Raises ValueError if not text."""
    @classmethod
    def create(cls, *, name: str, content: str | bytes | dict[str, Any] | list[Any] | BaseModel, description: str | None=None, derived_from: tuple[str, ...] | None=None, triggered_by: tuple[DocumentSha256, ...] | None=None, attachments: tuple[Attachment, ...] | None=None, summary: str | None=None) -> Self:
...
class PipelineFlow:
    ...
    def run(self, run_id: str, documents: Any, options: Any) -> Sequence[Document]:
...
def pipeline_test_context(run_id: str='test-run', store: DocumentStore | None=None, publisher: ResultPublisher | None=None) -> Generator[ExecutionContext, None, None]:

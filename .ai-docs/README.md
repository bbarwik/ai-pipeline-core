...
# ai-pipeline-core v0.15.1 — API Reference
...
class MemoryDatabase:
    ...
    async def find_latest_documents_by_derived_from(self, values: list[str], *, document_type: str | None=None, max_age: timedelta | None=None) -> dict[str, DocumentRecord]: ...
...
```python
async def reconstruct_lifecycle_events(reader: DatabaseReader, root_deployment_id: UUID) -> list[ReconstructedEvent]: ...
...
class Conversation(BaseModel, Generic[T]):
    ...
    async def send(self, content: ConversationContent, *, tools: list[Tool] | None=None, tool_choice: Literal['auto', 'required', 'none'] | None=None, max_tool_rounds: int=MAX_TOOL_ROUNDS_DEFAULT, purpose: str | None=None, expected_cost: float | None=None) -> Conversation[None]: ...
        """Send message, returns NEW Conversation with response."""
    async def send_spec(self, spec: PromptSpec[str], *, documents: Sequence[Document] | None=None, include_input_documents: bool=True, tools: list[Tool] | None=None, tool_choice: Literal['auto', 'required', 'none'] | None=None, max_tool_rounds: int=MAX_TOOL_ROUNDS_DEFAULT, purpose: str | None=None, expected_cost: float | None=None) -> Conversation[None]: ...

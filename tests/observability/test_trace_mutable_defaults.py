...
from ai_pipeline_core.observability.tracing import TraceInfo
...
class TestTraceInfoMutableDefaults:
    ...
    def test_multiple_instances_have_independent_metadata_dicts(self):
        ...
        assert trace1.metadata is not trace2.metadata, "TraceInfo instances are sharing the same metadata dict! This indicates a mutable default parameter bug."
    ...
    def test_multiple_instances_have_independent_tags_lists(self):
        ...
        assert trace1.tags is not trace2.tags, "TraceInfo instances are sharing the same tags list! This indicates a mutable default parameter bug."
    ...
    def test_mixing_default_and_explicit_metadata(self):
        ...
        for i in range(len(traces)):
            for j in range(i + 1, len(traces)):
                assert traces[i].metadata is not traces[j].metadata, f"TraceInfo instances {i} and {j} share the same metadata dict"
    ...
    def test_mixing_default_and_explicit_tags(self):
        ...
        for i in range(len(traces)):
            for j in range(i + 1, len(traces)):
                assert traces[i].tags is not traces[j].tags, f"TraceInfo instances {i} and {j} share the same tags list"

...
class TestLocalTraceWriter:
    ...
    def test_span_start_creates_directories(
        self, writer: LocalTraceWriter, config: TraceDebugConfig
    ) -> None:
        ...
        assert (trace_dir / "_events.jsonl").exists()

    def test_span_end_writes_metadata(
        self, writer: LocalTraceWriter, config: TraceDebugConfig
    ) -> None:
    ...
    def test_index_tracks_hierarchy(
        self, writer: LocalTraceWriter, config: TraceDebugConfig
    ) -> None:
    ...
    def test_event_log_append_only(
        ...
        """Test events are appended to JSONL log."""
        writer.on_span_start("trace001", "span1", None, "test")
        writer.on_span_start("trace001", "span2", "span1", "test2")

        time.sleep(0.2)

        trace_dirs = list(config.path.iterdir())
        trace_dir = trace_dirs[0]
        events_file = trace_dir / "_events.jsonl"

        lines = events_file.read_text().strip().split("\n")
        assert len(lines) >= 3  # trace_start, span_start, span_start

        # Check events are valid JSON
        for line in lines:
    ...
    def test_unset_status_treated_as_completed(
        self, writer: LocalTraceWriter, config: TraceDebugConfig
    ) -> None:

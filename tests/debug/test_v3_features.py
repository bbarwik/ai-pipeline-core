"""Tests for V3 tracing features: events_file_mode, wrapper merging, split indexes."""
...
from ai_pipeline_core.debug import LocalTraceWriter, TraceDebugConfig, WriteJob


class TestEventsFileMode:
    """Tests for events_file_mode configuration."""

    def test_events_mode_none(self, tmp_path: Path) -> None:
        """Test events_file_mode='none' writes no events.yaml files."""
        config = TraceDebugConfig(path=tmp_path, events_file_mode="none")
        ...
        try:
            ...
            writer.on_span_end(
                WriteJob(
                    ...
                    attributes={},
                    events=[],
                    status_code="ERROR",  # Even with error, no events.yaml
        ...
        # No events.yaml should exist
        assert not (span_path / "events.yaml").exists()

    def test_events_mode_errors_only_with_ok_status(self, tmp_path: Path) -> None:
        """Test events_file_mode='errors_only' with OK status writes no events.yaml."""
        config = TraceDebugConfig(path=tmp_path, events_file_mode="errors_only")
        writer = LocalTraceWriter(config)
        ...
        try:
            ...
            writer.on_span_end(
                WriteJob(
                    ...
                    attributes={},
                    events=[],
                    status_code="OK",  # OK status = no events
    ...
    def test_events_mode_errors_only_with_unset_status(self, tmp_path: Path) -> None:
        """Test events_file_mode='errors_only' with UNSET status writes no events.yaml.

        UNSET is used by Laminar @observe for normal completions, not errors.
        """
        config = TraceDebugConfig(path=tmp_path, events_file_mode="errors_only")
        writer = LocalTraceWriter(config)
        ...
        try:
            ...
            writer.on_span_end(
                WriteJob(
                    ...
                    attributes={},
                    events=[],
                    status_code="UNSET",  # UNSET = not an error
                    status_description=None,
                    start_time_ns=now_ns - 1_000_000_000,
                    end_time_ns=now_ns,
                )
        ...
        # UNSET should not trigger events.yaml
        assert not (span_path / "events.yaml").exists()

    def test_events_mode_errors_only_with_error_status(self, tmp_path: Path) -> None:
        """Test events_file_mode='errors_only' with ERROR status writes events.yaml."""
        ...
        try:
            ...
            # Create mock event
            mock_event = Mock()
            mock_event.name = "error"
            mock_event.timestamp = now_ns
            mock_event.attributes = {"error.type": "ValueError"}
            ...
            writer.on_span_end(
                WriteJob(
                    ...
                    parent_id=None,
                    attributes={},
                    events=[mock_event],  # Provide actual events
                    status_code="ERROR",  # ERROR = write events
                    status_description="Test error",
                    start_time_ns=now_ns - 1_000_000_000,
                    end_time_ns=now_ns,
                )
        ...
        # ERROR should trigger events.yaml
        assert (span_path / "events.yaml").exists()

    def test_events_mode_all(self, tmp_path: Path) -> None:
        """Test events_file_mode='all' always writes events.yaml."""
        from unittest.mock import Mock

        config = TraceDebugConfig(path=tmp_path, events_file_mode="all")
        writer = LocalTraceWriter(config)
        ...
        try:
            ...
            writer.on_span_end(
                WriteJob(
                    ...
                    attributes={},
                    events=[mock_event],  # Provide actual events
                    status_code="OK",  # Even with OK, write events
                    status_description=None,
                    start_time_ns=now_ns - 1_000_000_000,
                    end_time_ns=now_ns,
                )
        ...
        # All mode writes events.yaml even for OK
        assert (span_path / "events.yaml").exists()

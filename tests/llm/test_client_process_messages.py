...
class TestProcessMessages:
    ...
    def test_context_with_cache_control(self):
        """Test that only the last context message gets cache control."""
    ...
    def test_full_message_ordering(self):
        ...
        # Check ordering: system -> context (cache only on last) -> messages (no cache)
        assert result[0]["role"] == "system"
    ...
    def test_cache_control_only_on_last_context(self):
        ...
        # First user message does not get cache control
        assert result[0]["role"] == "user"
        assert "cache_control" not in result[0]

        # Assistant message does not get cache control
        assert result[1]["role"] == "assistant"
        assert "cache_control" not in result[1]

        # Last message (user) gets cache control
        assert result[2]["role"] == "user"
    ...
    def test_assistant_as_last_context_gets_cache(self):
        ...
        # Last context message (assistant) gets cache
        assert result[1]["role"] == "assistant"
        assert "cache_control" in result[1]
        assert result[1]["cache_control"]["type"] == "ephemeral"
        assert result[1]["cache_control"]["ttl"] == "5m"

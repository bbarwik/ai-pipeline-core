...
class TestDocumentValidation:
    ...
    def test_name_security_validation(self):
        ...
        with pytest.raises(DocumentNameError):
            FlowDocument(name="../etc/passwd", content=b"test")

        with pytest.raises(DocumentNameError):
            FlowDocument(name="..\\windows\\system32", content=b"test")
        ...
        with pytest.raises(DocumentNameError) as exc_info:
            FlowDocument(name="test.description.md", content=b"test")
        assert "cannot end with .description.md" in str(exc_info.value)

        # Empty or whitespace names should fail
        with pytest.raises(DocumentNameError):
            FlowDocument(name="", content=b"test")

        with pytest.raises(DocumentNameError):
            FlowDocument(name=" ", content=b"test")

        with pytest.raises(DocumentNameError):
            FlowDocument(name="test ", content=b"test")
...
class TestDocumentProperties:
    ...
    def test_id_determinism(self):
        ...
        doc1 = FlowDocument(name="test1.txt", content=content1)
        doc2 = FlowDocument(name="test2.txt", content=content1)
        assert doc1.id == doc2.id
        assert doc1.sha256 == doc2.sha256

        # Different content should produce different ID
        doc3 = FlowDocument(name="test3.txt", content=content2)
    ...
    def test_mime_detection_text(self):
        ...
        # Plain text
        doc = FlowDocument(name="test.txt", content=b"Hello, world!")
        ...
        # Markdown
        doc_md = FlowDocument(name="test.md", content=b"# Header\n\nContent")
        assert doc_md.is_text is True
        assert doc_md.mime_type == "text/markdown"

        # JSON
        doc_json = FlowDocument(name="test.json", content=b'{"key": "value"}')
    ...
    def test_mime_detection_binary(self):
        ...
        doc_png = FlowDocument(name="test.png", content=png_header)
        ...
        doc_pdf = FlowDocument(name="test.pdf", content=pdf_header)
    ...
    def test_as_text_method(self):
        ...
        doc = FlowDocument(name="test.txt", content=text_content.encode("utf-8"))

        assert doc.as_text() == text_content

        # Binary document should raise
        doc_binary = FlowDocument(name="test.bin", content=b"\x00\x01\x02\x03")
    ...
    def test_as_json_method(self):
        ...
        doc = FlowDocument(name="test.json", content=json.dumps(json_data).encode())

        assert doc.as_json() == json_data

        # Invalid JSON should raise
        doc_invalid = FlowDocument(name="test.txt", content=b"not json")
    ...
    def test_as_yaml_method(self):
        ...
        doc = FlowDocument(name="test.yaml", content=yaml_content.encode())
    ...
    def test_serialize_model(self):
        ...
        doc = FlowDocument(name="test.txt", content=b"Hello, world!", description="Test document")
    ...
    def test_from_dict(self):
        ...
        doc = FlowDocument.from_dict(data)
    ...
    def test_base_type_properties(self):
        ...
        task_doc = TaskDocument(name="test.txt", content=b"test")
    ...
    def test_should_be_cached(self):
        """Test should_be_cached property."""
        doc = FlowDocument(name="test.txt", content=b"test")
        assert doc.should_be_cached is False  # Default implementation

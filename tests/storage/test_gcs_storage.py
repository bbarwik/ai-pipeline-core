...
class TestGcsStorage:
    ...
    def mock_gcs_bucket(self):
        ...
        with patch("ai_pipeline_core.storage.storage.GcpCredentials") as mock_creds:
            with patch("ai_pipeline_core.storage.storage.GcsBucket") as mock:
                # Mock GcpCredentials to avoid authentication
                ...
                bucket_instance.gcp_credentials = mock_creds_instance
    ...
    async def test_init_with_credentials(self):
        ...
        with patch("ai_pipeline_core.storage.storage.GcpCredentials"):
            ...
            with patch("ai_pipeline_core.storage.storage.Path") as mock_path:
                with patch("ai_pipeline_core.storage.storage.GcpCredentials") as mock_creds:
                    with patch("ai_pipeline_core.storage.storage.GcsBucket"):
                        ...
                        # Should be called twice: once for file and once as fallback
                        assert mock_creds.call_count >= 1
                        # Check if it was called with service_account_file
                        calls = mock_creds.call_args_list
                        assert any(
                            call == ((), {"service_account_file": Path("/path/to/key.json")})
                            for call in calls
                        )
    ...
    def test_with_base(self):
        ...
        with patch("ai_pipeline_core.storage.storage.GcpCredentials") as mock_creds:
            with patch("ai_pipeline_core.storage.storage.GcsBucket") as mock_bucket_class:
                ...
                mock_bucket.gcp_credentials = mock_creds_instance
                ...
                mock_sub_bucket.gcp_credentials = mock_creds_instance
    ...
    async def test_retry_on_failure(self):
        ...
        with patch("ai_pipeline_core.storage.storage.GcpCredentials") as mock_creds:
            with patch("ai_pipeline_core.storage.storage.GcsBucket") as mock_bucket_class:
                ...
                mock_bucket.gcp_credentials = MagicMock()

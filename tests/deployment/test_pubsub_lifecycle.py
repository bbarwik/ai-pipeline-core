...
class OversizedResultDeployment(PipelineDeployment[FlowOptions, _OversizedResult]):
    ...
    @staticmethod
    def build_result(run_id: str, documents: list[Document], options: FlowOptions) -> _OversizedResult:
...
#   + (flow.started + task.started + task.completed + progress.STARTED + progress.COMPLETED + flow.completed) for flow 1
#   + (flow.started + progress.STARTED + flow.failed) for flow 2 + 1 run.failed
TWO_FLOW_FAILURE_EVENT_COUNT = 11

# Cancelled (1 flow): 1 run.started + flow.started + progress.STARTED + flow.failed + 1 run.failed
CANCELLED_EVENT_COUNT = 5

# build_result failure: same as success but run.failed instead of run.completed
BUILD_RESULT_FAILURE_EVENT_COUNT = 14
...
class TestPubsubLifecycle:
    ...
    async def test_successful_pipeline_publishes_complete_event_sequence(
        ...
        # 4 progress events (STARTED + COMPLETED per flow x 2 flows)

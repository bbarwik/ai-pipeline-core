...
class TestDeployerAgentConfig:
    ...
    def _make_deployer(self, pyproject_data: dict[str, Any]) -> Deployer:
        ...
        with patch("ai_pipeline_core.utils.deploy.settings") as mock_settings:
            mock_settings.prefect_api_url = "http://test.api"
            mock_settings.prefect_gcs_bucket = "test-bucket"
            mock_settings.prefect_work_pool_name = "default"
            mock_settings.prefect_work_queue_name = "default"
            ...
            with patch("ai_pipeline_core.utils.deploy.Path") as mock_path:
                ...
                with patch("builtins.open", create=True):
                    with patch(
                        "ai_pipeline_core.utils.deploy.tomllib.load", return_value=pyproject_data
                    ):
                        return Deployer()
...
class TestDeployerBuildAgentsDedup:
    ...
    def test_extra_vendor_skips_matching_vendor_tarball(self, tmp_path: Path):
        ...
        (tmp_path / "crypto_mcp" / "pyproject.toml").write_text(
            '[project]\nname = "crypto-mcp"\nversion = "0.2.0"\n'
        )
...
class TestBuildWheelFromSource:
    ...
    def test_successful_build(self, tmp_path: Path):
        ...
        with patch("ai_pipeline_core.utils.deploy.subprocess.run", side_effect=mock_run):
    ...
    def test_no_wheel_produced_raises(self, tmp_path: Path):
        ...
        with patch("ai_pipeline_core.utils.deploy.subprocess.run", return_value=mock_result):
...
cli_agents = pytest.importorskip("cli_agents", reason="cli_agents not installed")


class TestDeployerManifestCompatibility:
    """Test that Deployer's manifest JSON is compatible with AgentManifest schema.

    The Deployer writes plain dict (no cli-agents dependency).
    AgentManifest reads it at runtime on Prefect worker.
    This test validates they agree on the schema.
    """

    def test_manifest_json_validates_as_agent_manifest(self):
        """Deployer dict must parse as AgentManifest without error."""
        from cli_agents.deploy import AgentManifest

        manifest_dict = {
            "module_name": "agent_initial_research",
            "agent_wheel": "agent_initial_research-0.6.0-py3-none-any.whl",
            "cli_agents_wheel": "cli_agents-0.7.0-py3-none-any.whl",
            "vendor_packages": ["crypto_mcp-0.2.0-py3-none-any.whl"],
            "built_at": "2025-01-27T12:34:56+00:00",
        }

        parsed = AgentManifest.model_validate_json(json.dumps(manifest_dict))

        assert parsed.module_name == "agent_initial_research"
        assert parsed.agent_wheel == "agent_initial_research-0.6.0-py3-none-any.whl"
        assert parsed.cli_agents_wheel == "cli_agents-0.7.0-py3-none-any.whl"
        assert parsed.vendor_packages == ["crypto_mcp-0.2.0-py3-none-any.whl"]
        assert parsed.built_at == "2025-01-27T12:34:56+00:00"

    def test_manifest_without_optional_fields(self):
        """Manifest with only required fields must still parse."""
        from cli_agents.deploy import AgentManifest

        manifest_dict = {
            "module_name": "agent_test",
            "agent_wheel": "agent-1.0.whl",
            "cli_agents_wheel": "cli-1.0.whl",
        }

        parsed = AgentManifest.model_validate_json(json.dumps(manifest_dict))
        assert parsed.vendor_packages == []
